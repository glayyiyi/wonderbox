...
class upload
{
	var $bucket = 'wzdoss'; //阿里云OSS 的bucket
	var $oss_used = 0;  //是否使用OSS服务
    var $dst_img;// 目标文件
    var $h_src; // 图片资源句柄
    var $h_dst;// 新图句柄
    var $h_mask;// 水印句柄
	...
	var $file_type = array('jpg','gif','bmp','png','jpeg');//上传图片
	...
	function setData($data = array()){
		...
		$this->file_type = isset($data['file_type'])?$data['file_type']:array('jpg','gif','bmp','png','jpeg');//最大的图片
	...
	*/
	...
	function upfile($data = array()){
		...
		//print_r("new_dir" . $newDir);  //result: new_dir/data/htdocs/dai3/core/../data/upfiles/images/2012-07/18/
		//上次多个图片
		...
		if (is_array($_FILES[$file]['name'])){
			...
			foreach($_FILES[$file]['name'] as $i => $value){
				if ($value!=""){
					$this->img_type = strtolower(substr($_FILES[$file]['name'][$i],strripos($_FILES[$file]['name'][$i],'.')+1));
					...
					$newFile = $this->file_newname.$i.strrchr($_FILES[$file]['name'][$i],'.');//新文件名
					...
					//判读是否为为图片
					if($this->is_image($allFile)==false){
						@unlink($allFile);
						echo "<script>alert('资料上传失败，文件不是图片或已被破坏!');history.go(-1);</script>";
						exit;
					}
			...
			$this->img_type = strtolower(substr($_FILES[$file]['name'],strripos($_FILES[$file]['name'],'.')+1));
			if ($_FILES[$file]['size']==0)	return "";//文件不存在
			if(!in_array($this->img_type,$this->file_type)) $error = -3; //不是合法的扩展名
			if(strpos($_FILES[$file]['type'],'image')===false) $error = -4;//不是图片
			if($_FILES[$file]['size'] > $this->file_size*1024) $error = -5; //图片大小超出限制
			if($_FILES[$file]['error'] !=0 ) $error = -2;
			$this->_mkdirs($this->file_dir);//创建文件夹			
			//timest 2012-07-18 
			// 在 OSS上 新建文件夹
			if($this->oss_used==1){ 
				$oss_file_dir = date("Y-m",time())."/".date("d",time())."/";
				$oss_dir = 'data/upfiles/images/'.$oss_file_dir;
				$oss_service->create_object_dir($this->bucket,$oss_dir);
			}
			$newFile = $this->file_newname.strrchr($_FILES[$file]['name'],'.');//新文件名
			$oldFile = $_FILES[$file]['name'];//旧文件名
			$allFile = $newDir.$newFile; //新文件路径+ 新文件名 ,本地上的图片

			//print_r('allFile' . $allFile); allFile/data/htdocs/dai3/core/../data/upfiles/images/2012-07/19/2263_attestation_13426680195.png
			if ($error!=""){
				echo "<script>alert('".$err_var[$error]."');history.go(-1);</script>";
				exit;
			}
			if(function_exists('move_uploaded_file')){
				...
				//将上传的资料放到data下，然后再放到OSS中,然后删除data下的临时图片
				//$allFile 存放的是临时文件的地址
				if(!file_exists($allFile)){
					echo "<script>alert('资料上传失败，请重新上传!');history.go(-1);</script>";
					exit;
				}
				/*如果上传失败，继续上传 by:timest 2012-10-06*/
				if($this->oss_used==1){	
					$count_t = 0 ;
					//把图片上传到OSS上 params: bucket  to from
					$res = $oss_service->upload_file_by_file($this->bucket, $oss_dir.$newFile, $allFile);
					while( !$res->isOK()){
						$count_t++;
						if($count>10){ //上传10次依旧失败就停止上传
							echo "<script>alert('资料上传失败，请稍后重试!');history.go(-1);</script>";
							exit;
						}
						$res = $oss_service->upload_file_by_file($this->bucket, $oss_dir.$newFile, $allFile);
					}
			...
			//$this->_createMask();
			//@$this->_output();
			//图片上传到OSS当中，需要修改图片存放的URL(两处)  By:timest
			if ($error==""){
	...
	function litpic ($data = array()){
		$this->img_type = strtolower(substr($data['url'],strripos($data['url'],'.')+1));
		$this->setData($data);
	...
	function UpfileSwfupload($data = array()){
		...
		$newFile = $data['user_id']."_".$data['data_type']."_".time().rand(110,999).strrchr($_FILES[$file]['name'],'.');//新文件名
		$oldFile = $_FILES[$file]['name'];//旧文件名
		$allFile = $newDir.$newFile; //
	...
	function upfiles($data = array()){
		...
		foreach($_FILES[$file]['name'] as $i => $value){
			if ($value!=""){
				...
				$newFile = md5(time().rand(1,9)).$i.strrchr($_FILES[$file]['name'][$i],'.');//新文件名
				$oldFile = $_FILES[$file]['name'][$i];//旧文件名
				$allFile = $newDir.$newFile; //
				if ($error<0){
