...
class Mysql {
	...
	**/
	function db_query($sql="",$noreplace=""){
		if ($noreplace == ""){
			while (preg_match ('/{([a-zA-Z0-9_-]+)}/i', $sql, $regs)) {
			  $found = $regs[1];
			  $sql = preg_replace('/{'.$found.'}/i',$this->db_prefix.$found, $sql);
			}
		}
		// add by alpha for bug 6 begin 输出每次执行的原始sql
	...
	**/
	...
	function db_select($table,$where="",$field=""){
		if($field==""){
