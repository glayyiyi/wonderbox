...
class Magic 
{
	...
	function compile_file($template_name,$template_dir="",$type=""){
		global $cachepath;
		$htmlcache_path=$cachepath['html'].$template_name; //编译后的文件缓存完整路径
		...
		$_contents = $this->template_read($this->template_path);//读取模板的内容
		$contents = $this->compile_content($_contents,$template_name);//内容编译
		...
		echo $template_name;
		echo "--"; */
		/* 如果关闭缓存 or 开启但文件不存在 or 开启文件存在但过期 */
		if(!$cachepath['html_on']||($cachepath['html_on']&&!file_exists($htmlcache_path))||($cachepath['html_on']&&file_exists($htmlcache_path)&&(time()-filemtime($cachepath['html'].$template_name)>$cachepath['html_cachetime']))){
		//if(!file_exists($htmlcache_path)){
		//如果模板缓存文件不存在 或者 时间已经过期 则编译模板文件并缓存编译后的内容
	/* 	 echo "==";
		echo $this->compile_path;
	 echo "=="; */
		...
		/* 如果开启缓存模块 */
		//add by weego for 缓存模块 20130301
		if(in_array($template_name,$cachepath['html_type'])&&$cachepath['html_on']){
			file_put_contents($htmlcache_path, $contents);
		}
		ob_end_clean();
		...
		//直接输出模板编译后缓存的内容
		 ...
		 //add by weego for 缓存模块 20130301
		ob_start();
		$contents = file_get_contents($htmlcache_path);
		ob_end_clean();
	 echo "-----";
	 echo $htmlcache_path;
		echo $contents;  
	...
	function compile_content($content,$template_name="index.txt"){
	...
	function compile_tag($template_tag){
		...
		/* rewrite */
		elseif ($tag_command == 'rewrite'){
			$parse_var = $this->compile_parse_var($tag_str);
			return $this->compile_rewrite_tag($parse_var);
		}
		/* message */
		elseif ($tag_command == 'message'){
			$parse_var = $this->compile_parse_var($tag_str);
			return $this->compile_message_tag($parse_var);
		}
		//其他
		else{
			$parse_var = $this->compile_parse_var($tag_str);
	...
	*/
	 ...
	 * url重写
	 */
	function compile_rewrite_tag($parse_var){
		$_url = $parse_var['url'];
		$_url = str_replace('@@', '=', $_url);
		if($_url!=''){
			return "<? header('location:$_url');exit(); ?>";
		}else{
			...
			if($referer==''){
				...
				return "<? header('location:$referer');exit(); ?>";
			}
		}
	}
	function compile_message_tag($parse_var){
		$referer = $_SERVER['HTTP_REFERER']==''?$_SERVER['HTTP_HOST']:$_SERVER['HTTP_REFERER'];
		$content = $parse_var['content']==""?'系统错误':$parse_var['content'];
		$msg = $parse_var['msg']==""?'返回上一页':$parse_var['msg'];
		if($parse_var['url']==''){
			$_url = '<? ';
			$_url .= 'if($_SERVER[\'HTTP_REFERER\']==""){$this->magic_vars[\'_message\'][\'_uri\']=parse_url($_SERVER[\'SCRIPT_URI\']);$this->magic_vars[\'_message\'][\'_url\']=$this->magic_vars[\'_message\'][\'_uri\'][\'scheme\'].\'://\'.$this->magic_vars[\'_message\'][\'_uri\'][\'host\'];echo $this->magic_vars[\'_message\'][\'_url\'];}else{echo $_SERVER[\'HTTP_REFERER\'];}';
			$_url .= ' ?>';
			$url = 'if($_SERVER[\'HTTP_REFERER\']==""){$this->magic_vars[\'_message\'][\'_uri\']=parse_url($_SERVER[\'SCRIPT_URI\']);$this->magic_vars[\'_message\'][\'_uri\']=$this->magic_vars[\'_message\'][\'_uri\'][\'scheme\'].\'://\'.$this->magic_vars[\'_message\'][\'_uri\'][\'host\'];}else{$this->magic_vars[\'_message\'][\'_uri\']=$_SERVER[\'HTTP_REFERER\'];}';
		}else{
			$_url = $parse_var['url'];
			$_url = str_replace('@@', '=', $_url);
		}
		...
		if($parse_var['mode']==1){
			$_str = '<? ';
			$_str .= $url.';';
			$_str .= '$this->magic_vars[\'_message_\'][\'msg\']=\''.$content.'\';';
			$_str .= '$this->magic_vars[\'_message_\'][\'content\']=\''.$msg.'\';';
			$_str .= '$this->magic_vars[\'_message_\'][\'url\']=$this->magic_vars[\'_message\'][\'_uri\'];';
			$_str .= 'setcookie(\'message\', base64_encode(serialize($this->magic_vars[\'_message_\'])), time()+60 , \'/\');';
			$_str .= 'header("location:/index.php?message");';
			$_str .= 'exit();';
			$_str .= ' ?>';
			return $_str;
		}
