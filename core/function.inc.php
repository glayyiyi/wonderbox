...
//       $fp = fopen("http://key.erongdu.com/checkDymicPassword.html",r);
...
function otp_check_new($uchon_sn_db, $uchon_otp){

	require_once(ROOT_PATH ."core/json.class.php");

	$url="http://key.erongdu.com/checkDymicPassword2/sn_id/".$uchon_sn_db."/dym_password/".$uchon_otp.".html";
	$ch = curl_init($url);
...
$sms_type  em,normal
*/
function sendSMS($userid,$content,$system=0,$phone=0)
{
	global $_G,$mysql;
	...
	if ($_G['system']['con_issms']==1)
	{
		...
		$argv = array(
				...
				'content'=>'尊敬的【'.$realname.'】用户：'.date('Y-m-d H:i:s',time()).'，'.$content.'感谢您使用'.$_G['system']['con_webname'].'金融平台。【万得汇金】' ,//短信内容
				'ext'=>'',
...
function email_ciphertext($m,$type,$key=1){
	$yz = base64_encode('www.bddai.cn123');
	$yz_l = strlen($yz);
	for ($i=0; $i<$yz_l; $i+=2){
