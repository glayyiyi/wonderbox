<?php 
include_once "SecureTool.class.php";
 ...
 * 升级历史
 *
 * 1.0.2 修复返回报文没有正确urldecode的问题
 * 1.0.1 修复bgRetUrl参数拼写错误的问题。
 * 1.0.0 实现接口文档2.0.9所有接口
 *****************************************************
 *************************注意*************************
 * 该版本的php接口实现依赖安全签名机为商户和汇付天下之间的接口交互提供签名和验签方法，商户需要安装自己的安全签名机
 * 
 * 该版本的接口实现按照接口文档实现，其中针对 Version，CmdId和ChkValue 四个字段做了封装处理
 * 只要接口验签能够通过，商户可以不用关心这三个字段的实现方式。
 */
class Chinapnr{
	 ...
	 * @param string $merId
	 */
	private function Chinapnr($params){
...
HTML;
	...
	public static function getInstance(){
		if(self::$self == null){
			$params= require_once 'chinapnr.conf.php';
			self::$self= new Chinapnr($params);
		}
		return  self::$self;
	}
	 ...
	 * @return Ambigous <multitype:, NULL, mixed>
	 */
	public function queryBalanceBg($merCustId,$usrCustId)
	{
