...
class userClass extends friendsClass{
	...
	function GetSmsOnes($data){
		global $mysql;
		$_sql = " where 1=1 and apptypeid=1 ";
	...
	function AddSmsUser($data = array()){
		...
		if($account_result['use_money']>=$data["money"])
		{
		   ...
		   if($result==false || $re_log==false || $account_result==false){
	...
	function UpdateSmsUser($data = array()){
		...
		if($account_result==false || $re_log==false || $re_sql==false){
			mysql_query("rollback");
			return false;
		}else{
			mysql_query("commit");
