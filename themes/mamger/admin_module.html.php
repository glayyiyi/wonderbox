...
{elseif $_A.query_class=="channel"}
	...
	<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
		...
		<tr {if $key%2==1}class="tr2"{/if}>
			...
			<td align="center" ><a href="{$_A.admin_url}&q=module/fields&code={$item.code}">字段</a> | <a href="{$_A.admin_url}&q=module/channel/purview&code={$item.code}&id={$item.module_id}">权限</a> | <a href="{$_A.query_url}/edit&code={ $item.code}">编辑</a> |  <a href="#" onClick="javascript:if(confirm('请确定是否要卸载此模块，此模块卸载后所有的数据都将清空，请慎重处理')) location.href='{$_A.query_url}/del&code={$item.code}'">卸载</a> | <a href="{$_A.admin_url}&q=module/{$item.code}">内容管理</a></td>
		</tr>
		{/if}
		{/foreach}
		<tr>
	...
	{foreach from=$purviewlist key=key item=item}
	<div class="module_border">
		<div class="l">{$key}：</div>
		<div class="c">
			{$item}<a href="javascript:if(confirm('您确认要删除么?')) location.href='{$_A.query_url}/delpurview&code={$magic.request.code}&module_id={$module_id}&purviewcode={$key}'" style="margin-left:50px">删除</a>
		</div>
	</div>
	{/foreach}
	<form method="post" action="{$_A.query_url}/newpurview&code={$magic.request.code}">
