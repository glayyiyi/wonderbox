...
function checkEmail(email){
	...
	if (email == ''){
		...
		$.get("index.php?user", { q: "going/check_email", email: email},function (result){
			if ( result == true ){
...
function checkUsername(username){
	...
	if (username == ""){
		...
		$.get("index.php?user", { q: "going/check_username", username: username},function (result){
                if ( result == true ){
...
function checkRealname(realname){
	...
	document.getElementById('realname_notice').innerHTML = realname_default;
	realNameFlag = true;
	return true;
}
...
var email_msg_blank = "<font color=red>* 邮件地址不能为空</font>";
var email_msg_exist = "<font color=red>* 邮箱已存在,请重新输入</font>";
var email_msg_format = "<font color=red>* 邮件地址不合法</font>";
var password_msg_shorter = "<font color=red>* 登录密码不能少于 6 个字符。</font>";
...
var realname_default = "* (此处可不填)请填写您的真实姓名";
/* 会员修改密码 */
