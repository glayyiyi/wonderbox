...
if(file_exists(ROOT_PATH.'data/parameter/parameter_biaotype.php')){
	require ROOT_PATH.'data/parameter/parameter_biaotype.php';
}
//系统基本信息
if(file_exists(ROOT_PATH.'data/parameter/parameter_system.php')){
	require ROOT_PATH.'data/parameter/parameter_system.php';
}else{
	$system = array();
	$system_name = array();
...
if(file_exists(ROOT_PATH.'data/parameter/parameter_linkage.php')){
	require ROOT_PATH.'data/parameter/parameter_linkage.php';
}else{
...
//地区列表
if(file_exists(ROOT_PATH.'data/parameter/parameter_area.php')){
	require ROOT_PATH.'data/parameter/parameter_area.php';
...
if (file_exists(ROOT_PATH.'data/parameter/parameter_site.php')){
	require ROOT_PATH.'data/parameter/parameter_site.php';
}else{
...
$hackManArray=array('epage','keywords','code','type','area','account','branch','lilv','biaoType','q','page');
...
// 融盾安全监测防止SQL注入 20130923 
$get = array("page","epage");
foreach($get as $key=>$content){
    if(isset($_REQUEST[$get])){
        $_REQUEST[$key] = (int)$content;
    }
}
...
// 	}
		...
		if($_GET['biao_type']=='fast'){
			if($_G['user_result']['real_status'] == 1&&$_G['user_result']['scene_status'] == 1&&$_G['user_result']['video_status'] ==1){
			}else{
				echo '<script>alert("对不起，请先通过实名认证,现场认证及视频认证");</script>';
        ...
        if(isset($_GET['biao_type'])){
			...
			$biaotype_info= borrowClass::get_biao_type_info(array("biao_type"=>$_GET['biao_type']));

			if($biaotype_info){
				$_G['biaotype_info']=$biaotype_info;
			}else{
				$msg = array("您的输入有误,找不到相应的标种","<a href='/borrow/index.html'>返回标列表</a>");
				$_G['msg'] = $msg;
			}
         }
		if (isset($msg) && $msg!=""){
