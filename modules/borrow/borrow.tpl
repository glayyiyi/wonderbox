...
<div class="module_add">
	...
	<div class="module_border">
		...
		<div class="c">
			<input type="text" name="part_account" value="{$_A.borrow_result.part_account}" size="5" disabled="disabled"/> 元 <span>保留到“元”为单位。这里设置本次标的要奖励给所有投标用户的总金额。  </span>
		</div>
	</div>
	<div class="module_border">
		...
		<div class="c">
			<input type="text" name="funds" value="{$_A.borrow_result.funds}" size="5" disabled="disabled"/> %  <span>这里设置本次标的要奖励给所有投标用户的奖励比例。  </span>
		</div>
	</div>
	<div class="module_border">
	...
	<form action="" method="post">
		...
		<tr {if $key%2==1} class="tr2"{/if}>
			...
			{if $item.status ==0}<a href="{$_A.query_url}/view{$_A.site_url}&user_id={$item.user_id}&id={$item.id}">初审</a>{/if}
				...
				<a href="javascript:if(confirm('确定要停止流转此标么')) location.href='{$_A.query_url}/stoplz{$_A.site_url}&id={$item.id}';submit_fool()">停止流转</a>
				{/if}
			</td>
		</tr>
		{/foreach}
		...
		<div class="action">
			...
			<div class="floatr">
				...
				状态：<select id="status" ><option value="">全部</option><option value="1" {if $magic.request.status==1} selected="selected"{/if}>正在招标..</option><option value="2" {if $magic.request.status==2} selected="selected"{/if}>初审未通过</option><option value="3" {if $magic.request.status==3} selected="selected"{/if}>满标借款成功</option><option value="5" {if $magic.request.status=="5"} selected="selected"{/if}>被撤回</option><option value="4" {if $magic.request.status=="4"} selected="selected"{/if}>满标复审失败</option></select>
...
<table border="0" cellspacing="1" bgcolor="#CCCCCC" width="100%">
	<form action="" method="post">
		...
		<tr {if $key%2==1} class="tr2"{/if}>
			...
			<td>{if $item.status ==0}<a href="{$_A.query_url}/view{$_A.site_url}&user_id={$item.user_id}&id={$item.id}">初审</a>{/if}
				{if $item.status==1 && $item.biao_type!='lz'}
					{if $item.account>$item.account_yes}
					<a href="{$_A.query_url}/full_view{$_A.site_url}&id={$item.id}">撤回</a>
					{else}
