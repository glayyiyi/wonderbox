...
if($_G['user_id']!=""){
	...
	$_pur_var = array("site_list","module_all","system_all","bbs_all","attestation_list","borrow_list","account_list","userinfo_list","article_list");//是否有权限管理头部信息
...
//											$result_code = otp_check($uchon_sn_db, $uchon_otp);
                                           	...
                                           	$result_code = otp_check_new($uchon_sn_db, $uchon_otp);
//                                                                                        
											if($result_code->resCode == '1'){
												$uchon_stat='1';
											}else{//失败
//												$login_msg = "动态码错误 Code:" . $result_code->msg;
                                                $login_msg="动态密码不对";
												$uchon_stat='0';
											}
