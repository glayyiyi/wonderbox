...
class paymentClass {
	...
	public static function GetOne($data = array()){
		...
		if ($_result['type'] ==1 || isset($data['id']) && $data['id']!=""){
			if(isset($data['id']) && $data['id']!=""){
				$sql = "select * from `{payment}` where nid = '{$nid}' and id={$data['id']} and id={$data['id']}";
			}else{
				$sql = "select * from `{payment}` where nid = '{$nid}' and id={$data['id']}";
			}
	...
	function ToSubmit($data = array()){
		...
		$data['notify_url'] = $_G['system']['con_weburl']."modules/payment/notify2.php";//通知地址
		$data['return_url'] = $_G['system']['con_weburl']."modules/payment/return.php";//回调地址
		...
		$data['t_UsrCustId'] = $_G['user_result']['t_UsrCustId'];//p2p帐号唯一ID
