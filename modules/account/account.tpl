...
<table border="0" cellspacing="1" bgcolor="#CCCCCC" width="100%">
	  <form action="" method="post">
		...
		<tr {if $key%2==1} class="tr2"{/if}>
			<td>{$item.addtimes}</td>
			<td><a href="javascript:void(0)" onclick='tipsWindown("用户详细信息查看","url:get?index.php?admin&q=module/user/view&user_id={$item.user_id}&type=scene",500,230,"true","","true","text");'>{$item.usernames}</a></td>
			<td >{$item.money}</td>
		</tr>
		{/foreach}
		<tr>
...
<table border="0" cellspacing="1" bgcolor="#CCCCCC" width="100%">
	<form action="" method="post">
		...
		用户名：<input type="text" name="username" id="username" value="{$magic.request.username|urldecode}"/>
		状态：<select id="status"><option value="">全部</option><option value="1" {if $magic.request.status==1} selected="selected"{/if}>已通过</option><option value="0" {if $magic.request.status=="0"} selected="selected"{/if}>待审核</option><option value="2" {if $magic.request.status=="2"} selected="selected"{/if}>审核失败</option></select><input type="button" value="搜索" onclick="sousuo()" />
		</div>
		</td>
	</tr>
	<tr>
...
<div class="module_add">
	...
	<div class="module_border" >
		<div class="l">手续费:</div>
		<div class="c">
			<input type="text" name="fee" value="{ $_A.account_cash_result.fee}" size="5" onchange="updateFee({$_A.account_cash_result.total})" readonly="readonly" style="background:#CCCCCC">
			{literal}
			<script type="text/javascript">
...
<!--资金使用记录列表 结束-->
<!--回款续投奖励审核 开始-->
{elseif $_A.query_type=="return_reward"}
<table border="0" cellspacing="1" bgcolor="#CCCCCC" width="100%">
	<tr>
		<td width="" class="main_td">ID</td>
		<td width="" class="main_td">用户名</td>
	...
	{foreach from=$_A.return_reward key=key item=item}
	<tr {if $key%2==1} class="tr2"{/if}>
		<td>{$item.id}</td>
		<td class="main_td1"><a href="/{$_A.admin_url}&q=module/user/view&user_id={$item.user_id}&type=scene" class="thickbox" title="用户详细信息查看">{$item.username}</a></td>
		<td><a href="/invest/a{$item.borrow_id}.html" target="_blank">{$item.borrow_name}</a></td>
		<td>{$item.tender_account}</td>
		<td>{$item.can_reward}</td>
		<td>{$item.reward_scale}</td>
		<td>{$item.reward_account}</td>
		<td>{if $item.status==1}奖励发放成功{elseif $item.status==5}奖励发放失败{elseif $item.status==2}等待发放奖励{elseif $item.status==3}投标冻结中{/if}</td>
		<td>{$item.addtime|date_format:"Y-m-d H:i:s"}</td>
		<td>{if $item.status==2}<a href="{$_A.query_url}/return_rewardview&id={$item.id}&a=cash">审核</a>{else}--{/if}</td>
	</tr>
	{/foreach}
		<tr>
		<td colspan="12" class="action">
		<div class="floatr">
		状态：<select id="status"><option value="">全部</option><option value="2" {if $magic.get.status==2}selected="selected"{/if}>等待发放奖励</option><option value="1" {if $magic.get.status==1}selected="selected"{/if}>奖励发放成功</option><option value="5" {if $magic.get.status==5}selected="selected"{/if}>奖励发放失败</option><option value="3" {if $magic.get.status==3}selected="selected"{/if}>投标冻结中</option></select>
		用户名：<input type="text" name="username" id="username" value="{$magic.request.username|urldecode}"/>
		<input type="button" value="搜索" onclick="sousuo()" />
		</div>
		</td>
...
{elseif $_A.query_type=="return_rewardview"}
<div class="module_add">
	...
	<div class="module_border">
		<div class="l">金额：</div>
		<div class="c">{$_A.return_rewardview.reward_account}</div>
	</div>
	{if $_A.return_rewardview.status==2}
	<div class="module_border">
	...
	<div class="module_submit" >
		<input type="hidden" name="id" value="{$_A.return_rewardview.id}" />
	...
	<div class="module_border">
		<div class="l">是否发放：</div>
		<div class="c">
			{if $_A.return_rewardview.status==1}已发放{elseif $_A.return_rewardview.status==5}发放失败{/if}
		</div>
	</div>
	{/if}
	</form>
</div>
{elseif $_A.query_type=="return_account"}
<table border="0" cellspacing="1" bgcolor="#CCCCCC" width="100%">
	<tr>
		<td width="" class="main_td">ID</td>
		<td width="" class="main_td">用户名</td>
	...
	{foreach from=$_A.return_account key=key item=item}
	<tr {if $key%2==1} class="tr2"{/if}>
		<td>{$item.id}</td>
		<td class="main_td1"><a href="/{$_A.admin_url}&q=module/user/view&user_id={$item.user_id}&type=scene" class="thickbox" title="用户详细信息查看">{$item.username}</a></td>
		<td>{$item.total}</td>
		<td>{$item.use_money}</td>
		<td>{$item.already_use_money}</td>
		<td>{$item.not_use_money}</td>
		<td>{$item.addtime|date_format:"Y-m-d H:i:s"}</td>
	</tr>
	{/foreach}
		<tr>
...
{elseif $_A.query_type=="return_log"}
<table border="0" cellspacing="1" bgcolor="#CCCCCC" width="100%">
	<tr>
		<td width="" class="main_td">ID</td>
		<td width="" class="main_td">用户名</td>
	...
	{foreach from=$_A.return_log key=key item=item}
	<tr {if $key%2==1} class="tr2"{/if}>
		<td>{$item.id}</td>
		<td class="main_td1"><a href="/{$_A.admin_url}&q=module/user/view&user_id={$item.user_id}&type=scene" class="thickbox" title="用户详细信息查看">{$item.username}</a></td>
		<td>{$item.money}</td>
