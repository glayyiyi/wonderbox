...
<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
  <form action="" method="post">
	...
	<tr {if $key%2==1} class="tr2"{/if}>
		...
		<td class="main_td1" align="center"><a href="{$item.litpic}" target="_blank" ><img src="{$item.litpic}" width="50" height="50" style="border:1px solid #CCCCCC" /></a></td>
...
<table border="0" cellspacing="1" bgcolor="#CCCCCC" width="100%">
	...
	<tr {if $key%2==1} class="tr2"{/if}>
		...
		<td class="main_td1" align="center" >{if $item.real_status!=1}<a href="{$_A.query_url}/audit&user_id={$item.user_id}&type=realname"  class="thickbox" title="身份认证审核">审核</a>{else}-{/if} <input alt="{$_A.query_url}/realnameview&height=300&width=500&sfznum={$item.card_id}&truename={$item.realname}" class="thickbox" type="button" value="联网验证" /></td>
	</tr>
		{/foreach}
	<tr>
...
<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
	...
	<tr {if $key%2==1} class="tr2"{/if}>
		<td class="main_td1" align="center"><a href="{$_A.admin_url}&q=module/user/view&user_id={$item.user_id}&type=scene" class="thickbox" title="用户详细信息查看">{$item.username}</a></td>
		<td class="main_td1" align="center" >{$item.realname}</td>
		<td class="main_td1" align="center" >{if $item.real_status==2}<a href="{$_A.query_url}/audit&user_id={$item.user_id}&type=realname" class="thickbox" title="真实姓名审核"><font color="#FF0000">等待审核</font></a>{elseif $item.real_status==1}<font color="#009900">审核通过</font>{else}<font color="#cccccc">没有申请</font>{/if}</td>
		...
		<td class="main_td1" align="center" >{if $item.scene_status==0}<a href="{$_A.query_url}/audit&user_id={$item.user_id}&type=scene"  class="thickbox" title="现场认证审核"><font color="#FF0000">等待审核</font></a>{elseif $item.scene_status==1}<font color="#009900">审核通过</font>{else}<font color="#cccccc">没有申请</font>{/if}</td>
	</tr>
	{/foreach}
	<tr>
