...
elseif ($_A['query_type'] == "audit"){
	...
	if (isset($_POST['valicode']) && $_POST['valicode'] =="" ){
		$msg = array("验证码不能为空");
	  }elseif($_SESSION['valicode']!= strtolower($_POST['valicode'])){
			$msg = array("验证码不正确");
		}else{
			$_SESSION['valicode'] = "";
	...
	if(isset($_POST['vip_status'])){
		...
		if($result['vip_status']==1){
			...
			if ($data['vip_status']==1){
				...
				$message['status'] = 0;
				//messageClass::Add($message);//发送短消息
				$msg = array("VIP用户审核成功","","{$_A['query_url']}/vip&a=attestation");
