...
if (isset($_POST['password'])) {
	...
	if ($_POST['username'] == "") {
		...
		if(isset($_POST['remember']) && $_POST['remember']==1){
			$ctime = time()+intval($_POST['save_time']);
		}else{
			$ctime = 0;
		}
		if ($rdGlobal['uc_on']&&empty($index['openid']))
		{
			...
			if ($uid > 0) {
				...
				if(isset($result['serial_id']) && $result['serial_id']!='' ){
					...
					if( $is_used_uchon == 1 && otp_check_new($result['serial_id'], $_REQUEST['uchoncode'])->resCode!='1'){
...
// 				}
				...
				if(isset($result['serial_id']) && $result['serial_id']!='' ){
					...
					if( $is_used_uchon == 1 && otp_check_new($result['serial_idotp_check'], $_REQUEST['uchoncode'])->resCode!='1'){
						$msg = array("动态密码有误,请重试！", "", "?user&q=going/login&errror=" . $uid);
						return ;
					}
				}
